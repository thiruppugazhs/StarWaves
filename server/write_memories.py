...
lines = [
    ...
    'relevant memories; otherwise fall back to recent chronological.',
    '"""',
    '',
    'import hashlib',
    'import time',
    ...
    '    # P1: RAG path - semantic search when query present and pgvector available,',
    '    # served from a 30 s per-query cache to avoid repeated cosine searches',
    '    # for follow-up messages that share the same last user query.',
    '    if query:',
    '        try:',
    ...
    '            if memories:',
    '                memory_lines = "\n".join(f"- {m[chr(39)]content{chr(39)]}" for m in memories)',
    '                return (',
    '                    EVE_INSTRUCTIONS',
    ...
    '    memory_lines = "\n".join(f"- {m[chr(39)]content{chr(39)]}" for m in memories[:MAX_INJECTED_MEMORIES])',
    '    return (',
    '        EVE_INSTRUCTIONS',
...
ast.parse(content)
print("Syntax OK")
pathlib.Path("server/app/services/eve/memories.py").write_text(content, encoding="utf-8")
